...
2. **Completare formular:** alegere formular → motor `src/lib/forms/` (manifest versionat `selectManifest` → mapare declarativă profil→câmpuri → generare PDF cu pdf-lib) → validare → preview. Pachete livrate: **230** (felia 1), **dosar auto** (ITL-010/054/016/005 + DGPCI), **C168**, **impozit imobil** (ITL-001/003), **dosar copil** (alocație de stat + indemnizație creștere copil — datele copilului ca inputuri, fără a persista CNP-ul copilului), **petiții universale** (PetitionBuilder, OG 27/2002), **dosar șomaj** (înregistrare ANOFM + indemnizație, Legea 76/2002), **cadastru/CF** (cerere de înscriere în cartea funciară + extras, ANCPI, Legea 7/1996), **deces în familie** (ajutor de deces + pensie de urmaș, CNPP, Legea 360/2023 — datele decedatului ca inputuri, nepersistate), **PFA lifecycle** (înființare/mențiuni la ONRC, OUG 44/2008) și **urbanism/construcții** (certificat de urbanism + autorizație de construire, Legea 50/1991). Un pachet nou = un manifest înregistrat în `src/lib/forms/registered.ts`, fără cod de business nou în motor. Seturile de formulare se generează **atomic**: `generateAndFileForms` validează toate PDF-urile, apoi le persistă într-o singură tranzacție (`prisma.$transaction`) — niciun dosar orfan la eșec. Validarea inputurilor: Zod la granița rutei (format/enum) + reguli `validate` pe câmpuri în motor (`cnp`/`iban`/`date`).
